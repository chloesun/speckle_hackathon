...
export default class SectionBox {
  ...
  _setupControls() {
    ...
    this.controls.addEventListener(
      "change",
      this._draggingChangeHandler.bind(this)
    );
  ...
  _draggingChangeHandler() {
    ...
    if (this.dragging && !this.currentRange) {
      ...
      for (
        let i = 0;
        i < this.boxGeometry.attributes.position.array.length;
        i += 3
      ) {
        this.boxGeometry.attributes.position.array[i] += this.prevPosition.x;
        this.boxGeometry.attributes.position.array[i + 1] +=
          this.prevPosition.y;
        this.boxGeometry.attributes.position.array[i + 2] +=
          this.prevPosition.z;
      }
      this.boxGeometry.attributes.position.needsUpdate = true;
      this.boxGeometry.computeVertexNormals();
  ...
  _clickHandler(args) {
    ...
    let side =
      this.sidesSimple[`${args[0].face.a}${args[0].face.b}${args[0].face.c}`];
    ...
    for (let i = 0; i < planeArr.length; i++) {
      if (i % 3 === 0) {
        ...
        centre.add(
          new THREE.Vector3(tempArr[i - 2], tempArr[i - 1], tempArr[i])
        );
        index++;
      }
    }
  ...
  _generateSimpleCube(width = 0.5, depth = 0.5, height = 0.5) {
    ...
    const indexes = [
      0, 1, 3, 3, 1, 2, 1, 5, 2, 2, 5, 6, 5, 4, 6, 6, 4, 7, 4, 0, 7, 7, 0, 3, 3,
      2, 7, 7, 2, 6, 4, 5, 0, 0, 5, 1,
    ];
    ...
    if (targetBox) box = targetBox;
      ...
      if (this.viewer.interactions.selectedObjects.children.length !== 0) {
        box = new THREE.Box3().setFromObject(
          this.viewer.interactions.selectedObjects
        );
      } else if (
